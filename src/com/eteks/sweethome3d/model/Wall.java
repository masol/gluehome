...
public class Wall implements Serializable, Selectable, Elevatable {
  ...
  private boolean addedWall = true;
  
  private transient PropertyChangeSupport propertyChangeSupport = new PropertyChangeSupport(this);
  private transient float [][] pointsCache;

  public void setAdded (boolean newAdded){
	  this.addedWall = newAdded;
  }
  
  public boolean isNewAddedWall(){
	  return this.addedWall;
  }
   ...
   */  
  public float [][] getUnjoinedShapePoints() {
    if (this.arcExtent != null
        && this.arcExtent != 0
  ...
  /**
   * 提供获取wallLeftsSide的4个定点和中心点坐标的方法。
   * side = 0 为左侧墙5个点坐标；
   * side = 1 为右侧墙5个点坐标；
   */
  public float [][] getRoomSidePoints(int side){
	 float [][] points = getPoints();
	 float [][] sidePoints = new float[5][3];
	 for(int i = 0; i<2; i++){
		 for(int j = 0; j<2; j++){
			 if(side == 0){
				 sidePoints[i][j] = points[i][j];
				 sidePoints[3-i][j] = points[i][j];
			 }else{
				 sidePoints[i][j] = points[2+i][j];
				 sidePoints[3-i][j] = points[2+i][j];
			 }
			 sidePoints[i][2] = 0;
		 }
	 }
	 if(this.isTrapezoidal()){
		 sidePoints[2][2] = this.heightAtEnd;
	 }else{
		 sidePoints[2][2] = this.height;
	 }
	 sidePoints[3][2] = this.height;
	 if(this.isTrapezoidal()){
		 //TODO:梯形墙中心点的处理有待确定，暂时先用这种方法。
		 sidePoints[4][0] = (sidePoints[0][0]+sidePoints[1][0])/2;
		 sidePoints[4][1] = (sidePoints[0][1]+sidePoints[1][1])/2;
		 sidePoints[4][2] = (sidePoints[2][2]+sidePoints[3][2])/4;
	 }else{
		 sidePoints[4][0] = (sidePoints[0][0]+sidePoints[1][0])/2;
		 sidePoints[4][1] = (sidePoints[0][1]+sidePoints[1][1])/2;
		 sidePoints[4][2] = this.height/2;
	 }
	 return sidePoints;
  }
  /**
   ...
   * 以X轴为0度，沿逆时针方向为正方向。
   * 
   * 因为涉及到方向，所以按照向量来求解与x轴正方向的夹角
   * 夹角公式：cos（angle） = （x1*x2+y1*y2）/(sqrt(x1*x1+y1*y1)*sqrt(x2*x2+y2*y2))
   * 这里（x1,y1）为x轴正方向的单位向量，即（1,0）。
   * （x2,y2）为所要求解得的墙体的向量。
   * sqrt（x2*x2+y2*y2）即为墙体长度，所以使用与wall.getLength()得到。
   * 化简以后得到angle = acos(x2/wall.getLength())
   */
  public float getWallAngle() throws GlueException{
	  if(this.arcExtent == null || this.arcExtent == 0){
		  float x = this.xEnd-this.xStart;
		  if(getStartPointToEndPointDistance() > 0){
			  float angle = (float) Math.acos(x/getStartPointToEndPointDistance());
			  if(this.yEnd-this.yStart <= 0){
				  return angle;  //返回弧度
//				  return Math.rint(angle*180/Math.PI); //返回角度
			  }else{
				  return -angle;
//				  return Math.rint(-angle*180/Math.PI);
			  }
		  }else{
			  throw new GlueException("wall length can not be zero");
		  }
	  }else{
		  throw new GlueException("arcWall has no wallAngle");
	  }
  }
