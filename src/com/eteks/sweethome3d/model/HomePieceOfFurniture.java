...
import org.spolo.glue.FurnitureInfo;
...
public class HomePieceOfFurniture implements PieceOfFurniture, Serializable, Selectable, Elevatable {
  ...
  private boolean isSetW = false;
  private boolean isSetD = false;
   ...
   */
    ...
    if (piece instanceof HomePieceOfFurniture) {
      ...
      this.originaldepth = ((HomePieceOfFurniture) piece).getOriginalDepth();
      this.originalheight = ((HomePieceOfFurniture) piece).getOriginalHeight();
      this.originalwidth = ((HomePieceOfFurniture) piece).getOriginalWidth();
      this.catalogId = ((HomePieceOfFurniture)piece).getCatalogId();
      this.cludeCategory = ((HomePieceOfFurniture)piece).getCludeCategory();
      this.selftype = ((HomePieceOfFurniture)piece).getSelfType();
      this.type = ((HomePieceOfFurniture)piece).getType();
      this.feature = ((HomePieceOfFurniture)piece).getFeature();
      this.furnitureinfo = ((HomePieceOfFurniture)piece).getFurnitureinfo();
      ...
      if (piece instanceof CatalogPieceOfFurniture) {
        ...
        this.originaldepth = ((CatalogPieceOfFurniture) piece).getOriginalDepth();
        this.originalheight = ((CatalogPieceOfFurniture) piece).getOriginalHeight();
        this.originalwidth = ((CatalogPieceOfFurniture) piece).getOriginalWidth();
//        ((CatalogPieceOfFurniture) piece).setFurnitureInfo();
        this.cludeCategory = (((CatalogPieceOfFurniture) piece).getCludeCategory());
        this.type = (((CatalogPieceOfFurniture) piece).getType());
        this.feature = (((CatalogPieceOfFurniture) piece).getFeature());
        this.selftype = (((CatalogPieceOfFurniture) piece).getSelfType());
        this.furnitureinfo = (((CatalogPieceOfFurniture) piece).getFurnitureinfo());
        this.isTexture =(((CatalogPieceOfFurniture) piece).isTexture());
        this.filePath = ((CatalogPieceOfFurniture) piece).getPath();
      }      
      this.visible = true;
      this.x = this.width / 2;
      this.y = this.depth / 2;
    ...
    this.heightscale = this.height/this.originalheight;
    this.widthscale = this.width/this.originalwidth;
    this.depthscale = this.depth/this.originaldepth;
    this.swtichVisible = false;
  
  }
  
  public boolean isSwitchVisible(){
	  return this.swtichVisible;
  }
...
public void setTexture(boolean isTexture) {
  ...
  public void setSwitchVisible(boolean viOrInvi){
	  this.swtichVisible = viOrInvi;
  }

	public float getOriginalDepth() {
  ...
  public void setDepth(float depth) {
	...
	depthscale = depth/this.originaldepth;
  }
  ...
  public void setHeight(float height) {
    ...
    heightscale = this.height/this.originalheight;
  }
  ...
  public void setWidth(float width) {
    ...
    widthscale = this.width/this.originalwidth;
  }
  ...
  public static Comparator<HomePieceOfFurniture> getFurnitureComparator(SortableProperty property) {
	...
	/**
	 * 获取模型的云端分类，对应装修模板中的category属性
	 * @return
	 */
	public String getCludeCategory() {
		return cludeCategory;
	}

	/**
	 * 设置模型在装修模板中的category属性
	 * @param cludeCategory
	 */
	public void setCludeCategory(String cludeCategory) {
		this.cludeCategory = cludeCategory;
	}

	/**
	 * 获取模型的自动摆放规则，对应装修模板中的feature
	 * @return
	 */
	...
	/**
	 * 获取模型的类型标识，标识是家具还是贴图，对应装修模板中的type
	 * @return
	 */
