        ...
        try {
          ...
          // Ensure image will always be viewed in a 128x128 pixels cell
        	if (piece instanceof CatalogPieceOfFurniture) {
        		furnitureID = ((CatalogPieceOfFurniture) piece).getId();
        		if (((CatalogPieceOfFurniture) piece).getPath() == null) {
        			imagePath = GlueUtil.getJiajuDir() + File.separator + furnitureID + "_preview_0.jpg";
				}else {
        		imagePath = ((CatalogPieceOfFurniture) piece).getPath() + File.separator + furnitureID + "_preview_0.jpg";
				}
			}
          iconStream = piece.getIcon().openStream();
          System.out.println("In Furniture: furnitureID = " + furnitureID);
//          String imagePath = GlueUtil.getJiajuDir().toString() + File.separator + furnitureID + "_preview_0.jpg";
          File imageFile = new File(imagePath);
          System.out.println(imageFile.getAbsoluteFile().toString());
          if (imageFile.exists()) {
        	  System.out.println();
        	  image = ImageIO.read(new FileInputStream(imageFile));
		}else {
			imagePath = imagePath.replace("_preview_0", "");
...
//            this.pieceIconLabel.setIcon(IconManager.getInstance().getIcon(piece.getIcon(), height, this)); 
            ...
            this.pieceIconLabel.setIcon(icon);
          }
