...
public class FurnitureCatalogController implements Controller {
  ...
  public void dropFiles(List<String> importableModels) {
	 ...
	 * delete furniture
	 */
	public void deleteSelectedFurniture(Home home) {
		if (this.preferences != null) {
			if (this.selectedFurniture.size() > 0) {
				int num = this.selectedFurniture.size();
				List<CatalogPieceOfFurniture> pieces = this.selectedFurniture;
				Home tempHome = home.clone();
				List<HomePieceOfFurniture> oldFurnitures = tempHome.getFurniture();
				for (HomePieceOfFurniture f : oldFurnitures) {
					if (f instanceof HomeFurnitureGroup) {
						tempHome.deletePieceOfFurniture(f);
						List<HomePieceOfFurniture> furnitureWithinGroups = HomeFurnitureGroup.getFurnitureWithoutGroups(((HomeFurnitureGroup)f).getFurniture());
						for(HomePieceOfFurniture fg : furnitureWithinGroups){
							tempHome.addPieceOfFurniture(fg);
						}
					}
				}
				List<HomePieceOfFurniture> newFurnitures = tempHome.getFurniture();
				List<CatalogPieceOfFurniture> usedPieces = new ArrayList<CatalogPieceOfFurniture>();
				String usedInfo = "\u9700\u8981\u5220\u9664\u7684\u5BB6\u5177\u6B63\u5728\u573A\u666F\u4E2D\u4F7F\u7528\u3002"
						+ "\n" + "\u4E0D\u80FD\u76F4\u63A5\u5220\u9664\u3002";
				for(int i = 0;i < num; i++){
					CatalogPieceOfFurniture piece = pieces.get(i);
					for (HomePieceOfFurniture furniture : newFurnitures) {
						if (furniture.getCatalogId().equals(piece.getId())) {
							usedPieces.add(piece);
						}
					}
				}
				if (!usedPieces.isEmpty()) {
					JOptionPane.showMessageDialog(null, usedInfo, "\u8B66\u544A\uFF01", JOptionPane.WARNING_MESSAGE);
					return;
				} else {
					for (int i = 0; i < num; i++) {
						CatalogPieceOfFurniture piece = pieces.get(i);
						System.out.println(piece.getId());
					this.preferences.getFurnitureCatalog().delete(piece);
						String projectPath = null;
						System.out.println(getProjectName());
						if (getProjectName() == null) {
							projectPath = GlueUtil.getJiajuDir()
									.getAbsolutePath();
					} else {
							projectPath = GlueUtil.getJiajuDir()
									.getAbsolutePath()
						...
						if (projectDir.isDirectory()) {
							File[] models = projectDir.listFiles();
							int fileNum = models.length;
							for (int j = 0; j < fileNum; j++) {
								if (models[j].getAbsolutePath().contains(
										piece.getId())) {
									models[j].delete();
								}
							}
			...
			if (this.selectedFurniture.size() > 0) {
				...
				for(int i = 0;i < num; i++){
					...
					if (getProjectName() == null) {
						...
						projectPath = GlueUtil.getJiajuDir().getAbsolutePath() + File.separator +getProjectName();
					}
					System.out.println(projectPath);
					File projectDir = new File(projectPath);
					if (projectDir.isDirectory()) {
						...
						int fileNum = models.length;
						for (int j = 0; j < fileNum; j++) {
							if (models[j].getAbsolutePath().contains(piece.getId())) {
								models[j].delete();
							}
						}
					}
