...
import org.spolo.glue.smartlaytout.SmartLayout;
import org.spolo.glue.smartlaytout.SmartLayoutListener;
...
import com.eteks.sweethome3d.viewcontroller.WallController.WallShape;
...
public class RoomController implements Controller {
  ...
  private SmartLayoutListener m_SmartLayoutListener;

  private boolean isSmartLayout = false;
  ...
  public RoomController(final Home home,
    ...
    updateProperties();
    m_SmartLayoutListener = new SmartLayoutListener(this.home,this.preferences,this.contentManager);
  }
  public SmartLayoutListener getSmartLayoutListener(){
	 return this.m_SmartLayoutListener;
  }
  ...
  public DialogView getView() {
    ...
    m_SmartLayoutListener.setRoomController(this);
   ...
   */
  public List<WallSide> getRoomsWallSides(List<Room> rooms, List<WallSide> defaultWallSides) {
  ...
  public void modifyRooms() {
    ...
    if (!selectedRooms.isEmpty()) {
      ...
      //删除重叠的墙体。
      deleteDuplWall(selectedRooms, addedWalls);
  ...
  private void deleteDuplWall(List<Room> selectedRooms, List<ModifiedWall> addedWalls) {
	  List<WallSide> newSelectedRoomsWallSides = getRoomsWallSides(selectedRooms, null);
	  List<Wall> walls = new ArrayList<Wall>();
	  for (WallSide wallSide : newSelectedRoomsWallSides) {
			 walls.add(wallSide.getWall()); 
		 } 
	  List<Wall> tempWalls = new ArrayList<Wall>(); 
		 
		 Iterator<Wall> iteratorWall = walls.iterator();
		 while (iteratorWall.hasNext()) { 
			 Wall tempwall = (Wall) iteratorWall.next(); 
			 if (tempWalls.contains(tempwall)) { 
				 iteratorWall.remove(); 
			 }else { 
				 tempWalls.add(tempwall); 
			 } 
		 }
	  for (ModifiedWall addedWall : addedWalls) {
		  Wall newWall = addedWall.getWall();
		  float xStart = formatData(newWall.getXStart(), 2);
		  float yStart = formatData(newWall.getYStart(), 2);
		  float xEnd = formatData(newWall.getXEnd(), 2);
		  float yEnd = formatData(newWall.getYEnd(), 2);
		  System.out.println("原墙体起点终点坐标为：(" + xStart + "," + yStart + ") , (" + xEnd + "," + yEnd + ")");
 
			 for (Wall wall : walls) {
				 System.out.println("测试墙体起点终点坐标为：(" + wall.getXStart() + "," + wall.getYStart() + ") , (" + wall.getXEnd() + "," + wall.getYEnd() + ")");
				float [][] points = wall.getPoints();
				for (int i = 0; i < points.length; i++) {
					if (points[i][0] >= 0) {
						points[i][0] = (float) (formatData(points[i][0], 2) + 0.01);
					}else {
						points[i][0] = (float) (formatData(points[i][0], 2) - 0.01);
					}
					if (points[i][1] >= 0) {
						points[i][1] = (float) (formatData(points[i][1], 2) + 0.01);
					}else {
						points[i][1] = (float) (formatData(points[i][1], 2) - 0.01);
					}
				}
				Shape wallShape = getShape(points);
				  System.out.println("wallPoints");
				  for (int i = 0; i < points.length; i++) {
					System.out.println(points[i][0] + "," + points[i][1]);
				  }
				  System.out.println("wallshape contains" + xStart + "," + yStart + "is" + wallShape.contains(xStart, yStart));
				  System.out.println("wallshape contains" + xEnd + "," + yEnd + "is" + wallShape.contains(xEnd, yEnd));
				  if (wallShape.contains(xStart, yStart) && wallShape.contains(xEnd, yEnd) && !(newWall == wall)) {
  ...
  public class WallSide {
     ...
     * @throws Exception
     */
    public float [][] getWallSidePoints(){
		return this.wall.getRoomSidePoints(this.side);
    }
  }
   ...
   * being bound to controller and its view.
   */
  private  class SmartLayoutUndoableEdit extends AbstractUndoableEdit {
    private final Home                home;
    private final UserPreferences     preferences;
    private final List<HomePieceOfFurniture> smartlayoutFurnitures;
    private final LinkedList<SourceObject> sourceobjlist;
    private final List<ModifiedWallSide> modifiedWallSides;
    private final List<ModifiedRoom> modifiedRooms;
    private final SmartLayout sl;

    private SmartLayoutUndoableEdit(Home home,
                                          UserPreferences preferences,
                                          List<HomePieceOfFurniture> smartlayoutFurnitures,
                                          LinkedList<SourceObject> sourceobjlist ,
                                          List<ModifiedWallSide> modifiedWallSides,
                                          List<ModifiedRoom> modifiedRooms,
                                          SmartLayout sl
                                          ) {
      this.home = home;
      this.preferences = preferences;
      this.smartlayoutFurnitures = smartlayoutFurnitures;
      this.sourceobjlist = sourceobjlist;
      this.modifiedWallSides = modifiedWallSides;
      this.modifiedRooms = modifiedRooms;
      this.sl = sl;
      ;
    ...
    public void undo() throws CannotUndoException {
      super.undo();
      undoSmartLayout(this.home, this.preferences, this.smartlayoutFurnitures, this.sourceobjlist, this.modifiedWallSides, this.modifiedRooms, this.sl);
//      this.home.setSelectedItems(this.oldSelection);
    }
    ...
    public void redo() throws CannotRedoException {
      super.redo();
      doSmartLayout(this.home, this.preferences, this.smartlayoutFurnitures, this.sourceobjlist, this.modifiedWallSides, this.modifiedRooms, this.sl);
//      this.home.setSelectedItems(this.newSelection);
    }
  ...
  public void smartLayout(
	  ...
	  List<HomePieceOfFurniture> furnitures = new ArrayList<HomePieceOfFurniture>();
	  List<Selectable> oldSelection = this.home.getSelectedItems();
	  List<ModifiedRoom> modifiedRooms = new ArrayList<ModifiedRoom>();
	  List<Room> selectedRooms = Home.getRoomsSubList(oldSelection);
	  for (int i = 0; i < selectedRooms.size(); i++) {
			modifiedRooms.add(new ModifiedRoom(selectedRooms.get(i)));
	  }

	  //----------------------------------------------------
	  //重新计算墙体，避免摆放踢脚线和设置贴图是出错。
	  List<WallSide> selectedRoomsWallSides = getRoomsWallSides(selectedRooms, null);
      ...
      List<Selectable> newSelection = new ArrayList<Selectable>(oldSelection);
        ...
        deleteDuplWall(selectedRooms, addedWalls);
      List<ModifiedWallSide>  modifiedWallSides = new ArrayList<ModifiedWallSide>();

	  for(int i = 0; i < selectedRoomsWallSides.size(); i++){
		  modifiedWallSides.add(new ModifiedWallSide(selectedRoomsWallSides.get(i)));
	  }
	  //---------------------------------------------------

	  doSmartLayout(home, preferences, furnitures, sourceobjlist,modifiedWallSides, modifiedRooms, sl);
	  if(this.undoSupport != null){
		  UndoableEdit undoableEdit = new SmartLayoutUndoableEdit(home, preferences, furnitures, sourceobjlist, modifiedWallSides, modifiedRooms, sl);
		  this.undoSupport.postEdit(undoableEdit);
	  }
	  checkFurnitureInWall(home, modifiedRooms);
  }

  /**
   * 实现自动摆放和自动上贴图
   */
  private  void doSmartLayout(Home home,
		  UserPreferences preferences,
		  List<HomePieceOfFurniture> smartlayoutFurnitures,
		  LinkedList<SourceObject> sourceobjlist,
		  List<ModifiedWallSide> wallSides,
		  List<ModifiedRoom> modifiedRooms,
          SmartLayout sl) {
	  
	  //TODO 检测room中是否有踢脚线，并删除
	  checkExistFurnitures(home, modifiedRooms);
	  //TODO 删除墙体内部的踢脚线
	  checkFurnitureInWall(home, modifiedRooms);
	  List<List<HomePieceOfFurniture>> furnitures = new ArrayList<List<HomePieceOfFurniture>>();  //用于记录户型中自动添加的模型，模型替换和撤销时用到
	  for (SourceObject so : sourceobjlist) {
			GResult re = sl.autoLayout(so);
			if (so.getHomePieceOfFurniture() != null) {
				/**
				 * 删除原来Room中已经自动摆放的家具
				 */
				for (ModifiedRoom modifiedRoom : modifiedRooms) {
					List<List<HomePieceOfFurniture>> oldFurnitures = modifiedRoom.getRoom().getSmartLayoutElements();
					if (oldFurnitures != null) {
						for (List<HomePieceOfFurniture> oldFurniture : oldFurnitures) {
							for (HomePieceOfFurniture piece : oldFurniture) {
								home.deletePieceOfFurniture(piece);
							}
						}
					}
				}
				List<HomePieceOfFurniture> furniture = addCloudHomeFurniture(smartlayoutFurnitures, re, this.preferences);
				if (furniture != null) {
					
					//TODO 根据需求情况对不同个模型进行打组
					//TODO 1. 将自动摆放的吊顶打组，并命名。
					//注：打组必须在添加到furnitures之前
					HomePieceOfFurniture piece = furniture.get(0);
					if (piece.getSelfType() != null && piece.getSelfType().equals("none")) {
//						List<HomePieceOfFurniture>  furnituresGroup = new ArrayList<HomePieceOfFurniture>();
						HomeFurnitureGroup ceilingGroup = groupSelectedFurnitures(furniture, "\u540A\u9876");
						ceilingGroup.setMovable(true);
						ceilingGroup.setVisible(true);
						ceilingGroup.setAutoAdded(true);
//						furnituresGroup.add(ceilingGroup);
//						furniture.clear();
//						furniture.add(ceilingGroup);
//						furnitures.add(furnituresGroup);
					}else {
			...
			if(so.getHomeTexture() != null){
				setRoomWallTexture(so.getHomeTexture(), wallSides);
			}
			if (so.getFloorTexture() != null) {
				System.out.println("floor");
				for (ModifiedRoom modifiedRoom : modifiedRooms) {
					modifiedRoom.getRoom().setFloorTexture(so.getFloorTexture());
				}
//				getFloorTextureController().setTexture(so.getFloorTexture());
			}
			if (so.getCeilingTexture() != null) {
				System.out.println("ceiling ");
				for (ModifiedRoom modifiedRoom : modifiedRooms) {
					modifiedRoom.getRoom().setCeilingTexture(so.getCeilingTexture());
				}
//				getCeilingTextureController().setTexture(so.getCeilingTexture());
			}
		}
	  for (ModifiedRoom modifiedRoom : modifiedRooms) {
		  modifiedRoom.getRoom().setSmartLayoutElements(furnitures);
	}
  }
  /**
   * 
   */
 public void checkExistFurnitures(Home home, List<ModifiedRoom> modifiedRooms) {
	 List<HomePieceOfFurniture> furnitures = home.getFurniture();
	 for (ModifiedRoom modifiedRoom : modifiedRooms) {
		 Shape roomShape = modifiedRoom.getRoom().getShape();
		 for (HomePieceOfFurniture furniture : furnitures) {
			if (roomShape.contains(furniture.getX(), furniture.getY())
					&& furniture.getSelfType() != null && furniture.getSelfType().equals("skirtingboard")
					&&furniture.getFeature() != null && furniture.getFeature().equals("RandomCalculate")) {
 ...
 public void checkFurnitureInWall(Home home, List<ModifiedRoom> modifiedRooms) {
	 List<HomePieceOfFurniture> furnitures = home.getFurniture();
	 List<Room> rooms = new ArrayList<Room>();
	 List<WallSide> wallSides = new ArrayList<RoomController.WallSide>();
	 //这里先保留两种删除墙内踢脚线的方法，不知道那种效果更好，通过制作反馈来看改进。
	//只删除room相关的墙体中的踢脚线。
//	 rooms = home.getRooms();
//	 for (ModifiedRoom modifiedRoom : modifiedRooms) {
//		 rooms.add(modifiedRoom.getRoom());
//	 }
//	 wallSides = getRoomsWallSides(rooms, null);
	 //墙体去重，防止出现一堵墙出现两遍的情况。
...
//	 }
	 ...
	 for (Wall wall : walls) {
		for (HomePieceOfFurniture furniture : furnitures) {
			...
			Shape furnitureShape = getShape(furniture.getPoints());
			Rectangle2D furnitureBounds = furnitureShape.getBounds2D();
			if (wallShape.contains(furniture.getX(), furniture.getY())
					&& furniture.getSelfType() != null && furniture.getSelfType().equals("skirtingboard")
					&&furniture.getFeature() != null && furniture.getFeature().equals("RandomCalculate")) {
				home.deletePieceOfFurniture(furniture);
			}
   ...
   * Restores room properties from the values stored in <code>modifiedRooms</code> and <code>modifiedWallSides</code>.
   */
  private  void undoSmartLayout(Home home,
		  UserPreferences preferences,
		  List<HomePieceOfFurniture> smartlayoutFurnitures,
		  LinkedList<SourceObject> sourceobjlist ,
		  List<ModifiedWallSide> modifiedWallSides,
		  List<ModifiedRoom> modifiedRooms,
		  SmartLayout sl) {
	  for(HomePieceOfFurniture f : smartlayoutFurnitures){
		  home.deletePieceOfFurniture(f);
	  }
	  for(ModifiedWallSide t : modifiedWallSides){
		  t.reset();
	  }
	  for (ModifiedRoom m : modifiedRooms) {
		m.reset();
	}
  ...
  /**
	 ...
	 */
	public List<HomePieceOfFurniture> addCloudHomeFurniture(List<HomePieceOfFurniture> smartlayoutFurnitures, GResult furnitures,UserPreferences preferences) {
//		List<HomePieceOfFurniture> newFurniture = new ArrayList<HomePieceOfFurniture>();
		if (furnitures != null) {
			for (GPosition piece : furnitures.getPositions()) {
				...
				HomePieceOfFurniture homePiece = createHomePieceOfFurniture(piece.getFurniture());
				smartlayoutFurnitures.add(homePiece);
...
//				newFurniture.add(homePiece);
				this.home.addPieceOfFurniture(homePiece);
			}
			return smartlayoutFurnitures;
		} else {
	 ...
	 * @param texture
	 * @param wall
	 *            这个wall对象应该是需要在roomcontroller类里通过某些方法获得
	 * @param side
	 */
	public void setWallTexture(HomeTexture texture, Wall wall, int side) {
		if (texture != null) {
			if (side == 0) {
			...
			System.out.println("[ERROR]:The texture is null");
		}
	}

	public void setRoomWallTexture(HomeTexture texture, List<ModifiedWallSide> wallSides) {
		if (texture != null) {
//			WallSide wallside = wallSides.get(1);
			for (ModifiedWallSide wallside : wallSides) {
				setWallTexture(texture,
						wallside.getWallSide().getWall(), wallside.getWallSide().getSide());
			}
		}
	}
	public boolean isSmartLayout() {
		return isSmartLayout;
	}
	public void setSmartLayout(boolean isSmartLayout) {
		this.isSmartLayout = isSmartLayout;
	}
	
	public HomeFurnitureGroup groupSelectedFurnitures(List<HomePieceOfFurniture> furnitures, String groupName) {

			    HomeFurnitureGroup furnitureGroup = new HomeFurnitureGroup(furnitures, groupName);
			    HomePieceOfFurniture [] groupPieces =  furnitures.toArray(new HomePieceOfFurniture [furnitures.size()]);
			    for (HomePieceOfFurniture piece : groupPieces) {
					home.deletePieceOfFurniture(piece);
				}
			    home.addPieceOfFurniture(furnitureGroup);
			    return furnitureGroup;
	}
	public void setRoomSroundWallTexture(Room room, HomeTexture texture) {
		 List<Selectable> oldSelection = this.home.getSelectedItems();
		  List<ModifiedRoom> modifiedRooms = new ArrayList<ModifiedRoom>();
		  ...
		  setRoomWallTexture(texture, modifiedWallSides);
	}
}
